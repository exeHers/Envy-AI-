...
class LLMAdapter:
    ...
    def classify_intent(self, transcript: str) -> Dict[str, Any]:
        ...
        normalized = transcript_lower.replace(" dot pie", ".py").replace(" dot p y", ".py").replace(" dot pee", ".py")
        normalized = normalized.replace(" dot pi", ".py").replace(" dot pee", ".py")
        intent = "general"
        metadata: Dict[str, Any] = {}

        if "create" in normalized and "test.py" in normalized:
            intent = "skill:codeskill"
            metadata["action"] = "create_file"
            metadata["filename"] = "test.py"
            if "print" in normalized:
                metadata["content"] = 'print("hello from envy")\n'
        elif "create" in transcript_lower and "print" in transcript_lower:
            intent = "skill:codeskill"
            metadata["action"] = "create_file"
            metadata["filename"] = "test.py"
            metadata["content"] = 'print("hello from envy")\n'
